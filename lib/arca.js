...
export async function callWSFE(action, bodyContent, ambiente) {
  // servicios1 puede bloquear IPs de cloud; serviciosjava es alternativa
  const urls = ambiente === 'produccion'
    ? ['https://servicios1.afip.gov.ar/wsfev1/service.asmx', 'https://serviciosjava.afip.gob.ar/wsfev1/service.asmx']
    : ['https://wswhomo.afip.gov.ar/wsfev1/service.asmx']

  let lastErr = null
  for (const url of urls) {
    try {
      return await _callWSFE(url, action, bodyContent)
    } catch (e) {
      lastErr = e
    }
  }
...
async function _callWSFE(url, action, bodyContent) {
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ar="http://ar.gov.afip.dif.FEV1/">
  ...
  if (!resp.ok) {
    ...
    throw new Error(`WSFEv1 HTTP ${resp.status}: ${errText.slice(0, 200)}`)
  }
  return resp.text()
}
