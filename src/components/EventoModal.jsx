...
export default function EventoModal({ evento, eventos, config, productos = [], cajasAbiertas = [], onSave, onClose, addToast }) {
  ...
  const calc = useMemo(() => {
    ...
    const interes = intPct > 0 ? Math.round(montoParaInteres * intPct / 100) : 0
    const monto = parseFloat(form.monto) || 0
    return { base, mTot, eTot, artTot, dto, interes, intPct, total, monto, rest: Math.max(0, total - monto) }
  }, [form.chi, form.adu, form.promoId, form.interes, form.monto, form.pago, extraQtys, extraPrices, adHocExtras, articulosEvento, config, mrows, pChicoEv, pAdultoEv, esEdicionSena, esEdicionPagada, yaCobrado, adicionalSena, multiMet, metsPagados])
